...
def estilos_globales():
    ...
    <div class="footer-credits">
        <div style="font-weight: bold; color: #002F6C;">Hecho por: Caguana Martín, Cubillo Jeremy, Jaguaco Jhon, Morales David, Santacruz Elian.</div>
...
def sidebar_uce():
    with st.sidebar:
        ...
        st.divider()
        st.title("Navegación")
        opcion = st.radio("Ir a:", ["💬 Chat con Ing. Dante", "📂 Gestión de Bibliografía"])
        return opcion
...
def interfaz_chat():
    ...
    # === ENCABEZADO: Logo UCE | Título | Avatar Dante ===
    ...
    with col_titulo:
        st.markdown("""
            <div style="text-align: center; padding-top: 30px;">
                ...
                <p style='margin-top: 0px; color: gray; font-size: 16px;'>Ing. Dante - Tu Tutor Virtual de la FICA</p>
            </div>
    ...
    <div style="background-color: #f0f2f6; padding: 15px; border-radius: 5px; margin-bottom: 15px; font-size: 15px; border-left: 5px solid #C59200;">
        <strong>🦅 ¡Hola! Soy el Ing. Dante.</strong><br>
    ...
    if st.session_state.messages and st.session_state.messages[-1]["role"] == "user":
        with contenedor_chat:
            with st.chat_message("assistant", avatar=avatar_bot):
                ...
                placeholder.markdown("🦅 *Dante analizando archivos...*")
                try:
                    textos, fuentes = leer_pdfs_locales()
                    contexto = buscar_informacion(st.session_state.messages[-1]["content"], textos, fuentes)
                    model = genai.GenerativeModel(modelo)
                    response = model.generate_content(f"Eres el Ing. Dante de la FICA-UCE. Contexto: {contexto}. Pregunta: {st.session_state.messages[-1]['content']}")
